...
│   ├── Services/
│   │   └── PlayerService.cs            # 示例: 玩家服务 (管理 Model 生命周期)
│   ├── Presenters/
│   │   └── PlayerPresenter.cs          # 示例: 玩家 Presenter
│   ├── Views/
│   │   ├── IPlayerView.cs              # 示例: View 接口契约
│   │   └── PlayerView.cs               # 示例: MonoBehaviour View
│   └── Scenes/
│       └── GameScene/
...
### 6.1 View 注入 Presenter（子 Scope 模式）
...
### 6.2 Service 管理 Model 生命周期

- **Service 负责：** Model 创建、缓存、持久化、跨 Presenter 共享
- **Presenter 负责：** 订阅 Model 响应式属性，调用 View 接口更新 UI
- **DI 连接：** 通过工厂注册 `builder.Register<PlayerModel>(resolver => ...)` 从 Service 获取 Model
...
3. **创建 Service** — `Assets/HotUpdate/Services/ShopService.cs`，实现 `IShopService : IService`
...
7. **注册到 DI** — `GameInstaller.cs` 注册 Service，`GameSceneRoot.cs` 注册 Presenter/Model
...
| `GameTest.unity` | 1 | PlayerView UI 测试场景，挂载 GameSceneRoot |
